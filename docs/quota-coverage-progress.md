...
| --- | ---: | ---: |
| total | 12,081 | 10,398 |
| implemented | 2,532 | 2,076 |
| compatibleMetric | 2,535 | 2,535 |
| covered | 4,913 | 4,457 |
| uncovered | 7,168 | 5,941 |
| unmeasurable | 5,013 | 3,055 |
| measurable | 7,068 | 7,343 |

Implemented measurement availability: **40.67%** of the whole union, or
**69.51%** of the 7,068 quotas whose usage can be counted at all.
...
2,155 quotas are measurable and still uncovered. Sorting them by what their
names describe shows what the remaining work actually is:

| Shape | Quotas | What it would take |
| --- | ---: | --- |
| countable | 1,279 | the name describes a count; whether an API exposes that inventory has to be checked quota by quota |
| size or period | 575 | the bound applies to one payload, document or retention period, so there is a value to read only while a request is in flight |
...
- **Bedrock (229)** is per-model: `Records per batch inference job for <model>`,
...
- **Connect (35)** is workforce management: staffing groups, shift profiles,
  ...
  equivalent.
- **Pinpoint (33)** is almost entirely per-request: attribute counts, template
  character counts and payload sizes that exist only while a call is in flight.
- **lookoutmetrics (23)** and **iotevents (19)** have no client at all.
