    ...
    try:
        ...
        if INVOKE_ASYNC:
            ...
            if not REPORT_BUCKET:
                ...
                return False
            s3_location = wait_for_latest_report(
                session,
                REPORT_BUCKET,
                POLL_TIMEOUT_SEC,
                created_after=invocation_started_at
            )
            if s3_location:
...
def wait_for_latest_report(session, bucket, timeout_sec=1200, prefix='reports/', created_after=None):
    """Poll S3 for the newest report object and return its s3:// location.

    If *created_after* is provided, only objects newer than that timestamp are
    considered. This avoids picking up stale reports from previous runs.
    """
    s3_client = session.client('s3', region_name=REGION)
    start = time.time()
    latest_key = None
    ...
    while time.time() - start < timeout_sec:
        ...
        if contents:
            if created_after is not None:
                contents = [obj for obj in contents if obj.get('LastModified') and obj['LastModified'] >= created_after]
            if not contents:
                time.sleep(10)
                continue
