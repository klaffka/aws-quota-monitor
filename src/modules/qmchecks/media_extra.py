...
# An origin endpoint publishes manifests of four kinds and the quota bounds
# them together, so the listing's four lists are counted as one total.
MANIFEST_KINDS = ('HlsManifests', 'LowLatencyHlsManifests', 'DashManifests',
                  'MssManifests')


def max_manifests_per_endpoint(c):
    values = []
    for group in c.call('mediapackagev2', 'list_channel_groups', 'Items'):
        group_name = group.get('ChannelGroupName')
        for channel in c.call('mediapackagev2', 'list_channels', 'Items',
            ...
            for endpoint in c.call('mediapackagev2', 'list_origin_endpoints', 'Items',
                ...
                usage = sum(len(endpoint.get(kind) or ()) for kind in MANIFEST_KINDS)
...
def get_current_quotastatus_media_extra(session=None, *, ctx=None, skip=()):
    ...
    if any(service == 'mediapackagev2' for service, _ in context.quotas):
        entries.extend(context.run('mediapackagev2', MEDIAPACKAGEV2_CHECKS, skip))
    return entries
