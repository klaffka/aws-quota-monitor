...
Service Quotas and IAM call the service `elasticmapreduce`; the SDK client
is named `emr`.
"""
...
def active_instances_per_instance_group(ctx):
    values = []
    for cluster in ctx.call('emr', 'list_clusters', 'Clusters'):
        cluster_id = cluster.get('Id')
        if not cluster_id:
            continue
        for group in ctx.call('emr', 'list_instance_groups', 'InstanceGroups', ClusterId=cluster_id):
...
CHECKS = [
    ('L-1EE7982C', 'Maximum number of active clusters',
     lambda ctx: dict(usage=len(ctx.call('emr', 'list_clusters', 'Clusters',
